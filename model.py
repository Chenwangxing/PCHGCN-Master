import torch
import torch.nn as nn
from torch.nn import functional as F
import numpy as np
import math


device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


class SelfAttention(nn.Module):
    def __init__(self, in_dims=2, d_model=64, num_heads=4):
        super(SelfAttention, self).__init__()
        self.embedding = nn.Linear(in_dims, d_model)
        self.query = nn.Linear(d_model, d_model)
        self.key = nn.Linear(d_model, d_model)
        self.scaled_factor = torch.sqrt(torch.Tensor([d_model])).cuda()
        self.softmax = nn.Softmax(dim=-1)
        self.num_heads = num_heads
    def split_heads(self, x):
        # x [batch_size seq_len d_model]
        x = x.reshape(x.shape[0], -1, self.num_heads, x.shape[-1] // self.num_heads).contiguous()
        return x.permute(0, 2, 1, 3)  # [batch_size nun_heads seq_len depth]
    def forward(self, x, mask=False, multi_head=False):
        # batch_size seq_len 2
        assert len(x.shape) == 3
        embeddings = self.embedding(x)  # batch_size seq_len d_model
        query = self.query(embeddings)  # batch_size seq_len d_model
        key = self.key(embeddings)      # batch_size seq_len d_model
        if multi_head:
            query = self.split_heads(query)  # B num_heads seq_len d_model
            key = self.split_heads(key)  # B num_heads seq_len d_model
            attention = torch.matmul(query, key.permute(0, 1, 3, 2))  # (batch_size, num_heads, seq_len, seq_len)
        else:
            attention = torch.matmul(query, key.permute(0, 2, 1))  # (batch_size, seq_len, seq_len)
        attention = self.softmax(attention / self.scaled_factor)
        if mask is True:
            mask = torch.ones_like(attention)
            attention = attention * torch.tril(mask)
        return attention



##  Graph input + Physical constraint module
class SparseWeightedAdjacency(nn.Module):
    def __init__(self, spa_in_dims=2, embedding_dims=64, dropout=0,):
        super(SparseWeightedAdjacency, self).__init__()
        # dense interaction
        self.spatial_attention = SelfAttention(spa_in_dims, embedding_dims)
        self.dropout = dropout
        self.spa_softmax = nn.Softmax(dim=-1)

    # 计算距离变化率
    def compute_distance_rate_of_change(self, positions, velocities):
        # Calculate relative position 计算相对位置 (T, N, N, 2)
        relative_positions = positions.unsqueeze(2) - positions.unsqueeze(1)  # 每个行人之间的相对位置
        # Calculating relative speed 计算相对速度 (T, N, N, 2)
        relative_velocities = velocities.unsqueeze(2) - velocities.unsqueeze(1)  # 每个行人之间的相对速度
        # Calculating distance 计算距离 d (T, N, N)
        distances = torch.norm(relative_positions, dim=-1)  # 计算相对位置的范数（即两行人之间的距离）
        # Avoid division by zero by replacing the zero distance with a small value
        # 避免除以零的情况，将零距离替换为一个小值 (epsilon)
        epsilon = 1e-8
        distances = torch.where(distances == 0, torch.tensor(epsilon, dtype=distances.dtype), distances)
        # Calculate the rate of change of distance 计算距离变化率 dd/dt (T, N, N)
        distance_rate_of_change = torch.sum(relative_positions * relative_velocities, dim=-1) / distances
        return distance_rate_of_change

    def forward(self, graph, identity, obs_traj):
        assert len(graph.shape) == 3

        spatial_graph = graph[:, :, 1:]  # (T N 2)
        # obs_traj=[N, 2, T]    obs_traj.permute(2, 0, 1)=[T, N, 2]    spatial_graph=[T, N, 2]

        ## Distance Rate Of Change=[T, N, N]
        rate_of_change = self.compute_distance_rate_of_change(obs_traj.permute(2, 0, 1), spatial_graph)
        rate_of_change = torch.where(rate_of_change < 0, 1, 0)  #
        rate_of_change = rate_of_change.unsqueeze(1).to(device)
        spatial_similarity1 = rate_of_change.repeat(1, 4, 1, 1).float()
        spatial_similarity1 = spatial_similarity1 + identity[0].unsqueeze(1)
        # [T 4 N N]

        # ## Distance ##
        dis_graph = obs_traj.unsqueeze(0) - obs_traj.unsqueeze(1)
        dis_graph = dis_graph.permute(3, 0, 1, 2)
        distance = torch.norm(dis_graph, dim=3)
        spatial_distance = torch.where(distance < 5, 1, 0)  # spatial distance threshold = 5
        spatial_distance = spatial_distance.unsqueeze(1).to(device)
        spatial_distance = spatial_distance.repeat(1, 4, 1, 1).float()
        # [T 4 N N]

        spatial_similarity = torch.zeros(spatial_graph.shape[0], spatial_graph.shape[1],spatial_graph.shape[1])  # (T N N)
        ## View Degress ##
        for i in range(spatial_graph.shape[0]):
            spatial_visual = spatial_graph[i, :, :]
            dis_visual = dis_graph[i, :, :, :]
            dis_similarity = torch.cosine_similarity(spatial_visual.unsqueeze(1), dis_visual, dim=-1)
            spatial_similarity[i, :, :] = dis_similarity  # spatial_similarity (T N N)
        spatial_similarity = spatial_similarity + torch.eye(spatial_graph.shape[1]).unsqueeze(0)
        spatial_similarity = spatial_similarity.to(device)
        spatial_degrees = torch.acos(spatial_similarity.clamp(-1, 1))
        angles_degrees = spatial_degrees * 180.0 / math.pi
        spatial_similarity = torch.where(angles_degrees.unsqueeze(1) < 60, 1, 0)   # spatial view threshold = 10
        spatial_similarity = spatial_similarity.repeat(1, 4, 1, 1)
        spatial_similarity2 = spatial_similarity + identity[0].unsqueeze(1)
        #################################

        spatial_similarity = spatial_similarity1 * spatial_distance * spatial_similarity2

        # (T num_heads N N)   (T N d_model)
        dense_spatial_interaction = self.spatial_attention(spatial_graph, multi_head=True) + identity[0].unsqueeze(1)

        return dense_spatial_interaction * spatial_similarity, spatial_similarity



class GraphConvolution(nn.Module):
    def __init__(self, in_dims=2, embedding_dims=16, dropout=0):
        super(GraphConvolution, self).__init__()
        self.embedding = nn.Linear(in_dims, embedding_dims, bias=False)
        self.activation = nn.PReLU()
        self.dropout = dropout
    def forward(self, graph, adjacency):
        # graph [batch_size 1 seq_len 2]
        # adjacency [batch_size num_heads seq_len seq_len]
        gcn_features = self.embedding(torch.matmul(adjacency, graph))
        gcn_features = F.dropout(self.activation(gcn_features), p=self.dropout)

        return gcn_features  # [batch_size num_heads seq_len hidden_size]



class GraphCOM(nn.Module):
    def __init__(self):
        super(GraphCOM, self).__init__()
        self.embedding = nn.Conv2d(4, 1, 1, padding=0)
        self.activation = nn.PReLU()
    def forward(self, feature):
        gcn_features = self.embedding(feature)
        gcn_features = self.activation(gcn_features)
        return gcn_features  # [batch_size num_heads seq_len hidden_size]


class GraphSelfCOM(nn.Module):
    def __init__(self):
        super(GraphSelfCOM, self).__init__()
        self.embedding = nn.Conv2d(4, 4, 1, padding=0)
        self.activation = nn.PReLU()
    def forward(self, feature):
        gcn_features = self.embedding(feature)
        gcn_features = self.activation(gcn_features) + feature
        return gcn_features  # [batch_size num_heads seq_len hidden_size]


#  Gated fusion module
class AdaptiveWeightedSum(nn.Module):
    def __init__(self):
        super(AdaptiveWeightedSum, self).__init__()
        # 初始化权重张量并设置为可学习的参数
        self.COM = nn.ModuleList()
        self.COM.append(GraphCOM())
        self.COM.append(GraphCOM())
        self.COM.append(GraphCOM())

        self.SelfCOM = nn.ModuleList()
        self.SelfCOM.append(GraphSelfCOM())
        self.SelfCOM.append(GraphSelfCOM())
        self.SelfCOM.append(GraphSelfCOM())

        self.softmax_weight = nn.Softmax(dim=1)
    def forward(self, features1,  features2,  features3):  # [T, heads, N, 16]
        feat1 = self.COM[0](features1)   # (; 1; ;)
        feat2 = self.COM[1](features2)   # (; 1; ;)
        feat3 = self.COM[2](features3)   # (; 1; ;)
        feat = torch.cat((feat1, feat2, feat3), dim=1)   # (; 2; ;)
        feat = self.softmax_weight(feat)
        weighted_sum = (feat[:,0,:,:].unsqueeze(dim=1) * self.SelfCOM[0](features1)
                        + feat[:,1,:,:].unsqueeze(dim=1) * self.SelfCOM[1](features2)
                        + feat[:,2,:,:].unsqueeze(dim=1) * self.SelfCOM[2](features3))
        return weighted_sum



class SparseGraphConvolution(nn.Module):
    def __init__(self, in_dims=16, embedding_dims=16, dropout=0):
        super(SparseGraphConvolution, self).__init__()
        self.dropout = dropout

        self.spatial_temporal_sparse_gcn = nn.ModuleList()
        self.spatial_temporal_sparse_gcn.append(GraphConvolution(in_dims, embedding_dims))
        self.spatial_temporal_sparse_gcn.append(GraphConvolution(in_dims, embedding_dims))
        self.spatial_temporal_sparse_gcn.append(GraphConvolution(in_dims, embedding_dims))

        self.Adap_Weight = AdaptiveWeightedSum()

    def forward(self, graph, normalized_spatial_adjacency_matrix, spatial_similarity):
        # graph [1 seq_len num_pedestrians  3]
        # _matrix [batch num_heads seq_len seq_len]
        graph = graph[:, :, :, 1:]
        spa_graph = graph.permute(1, 0, 2, 3)  # (seq_len 1 num_p 2)

        ### Matrix power operations
        normalized_spatial_adjacency_matrix2 = torch.linalg.matrix_power(normalized_spatial_adjacency_matrix, 2)
        normalized_spatial_adjacency_matrix3 = torch.linalg.matrix_power(normalized_spatial_adjacency_matrix, 3)

        # gcn_features [T, heads, N, 16]
        gcn_features1 = self.spatial_temporal_sparse_gcn[0](spa_graph, normalized_spatial_adjacency_matrix)
        gcn_features2 = self.spatial_temporal_sparse_gcn[1](spa_graph, normalized_spatial_adjacency_matrix2*spatial_similarity)
        gcn_features3 = self.spatial_temporal_sparse_gcn[2](spa_graph, normalized_spatial_adjacency_matrix3*spatial_similarity)

        gcn_spatial_features = self.Adap_Weight(gcn_features1, gcn_features2, gcn_features3)

        gcn_spatial_features = gcn_spatial_features.permute(2, 1, 0, 3)

        return gcn_spatial_features


class TrajectoryModel(nn.Module):
    def __init__(self,embedding_dims=64, number_gcn_layers=1, dropout=0,obs_len=8, pred_len=12, n_tcn=5, num_heads=4):
        super(TrajectoryModel, self).__init__()

        self.number_gcn_layers = number_gcn_layers
        self.n_tcn = n_tcn
        self.dropout = dropout

        # sparse graph learning
        self.sparse_weighted_adjacency_matrices = SparseWeightedAdjacency()

        # graph convolution
        self.stsgcn = SparseGraphConvolution(in_dims=2, embedding_dims=embedding_dims // num_heads, dropout=dropout)

        self.tcns = nn.ModuleList()
        self.tcns.append(nn.Sequential(nn.Conv2d(obs_len, pred_len, 3, padding=1),
            nn.PReLU()))
        for j in range(1, self.n_tcn):
            self.tcns.append(nn.Sequential(nn.Conv2d(pred_len, pred_len, 3, padding=1),
                nn.PReLU()))

        self.output = nn.Linear(embedding_dims // num_heads, 2)
        self.multi_output = nn.Sequential(nn.Conv2d(num_heads, 16, 1, padding=0),
            nn.PReLU(),
            nn.Conv2d(16, 20, 1, padding=0),)

    def forward(self, graph, identity, obs_traj):
        # graph 1 obs_len N 3
        # obs_traj 1 obs_len N 2

        normalized_spatial_adjacency_matrix, spatial_similarity = self.sparse_weighted_adjacency_matrices(graph.squeeze(), identity, obs_traj.squeeze())

        gcn_representation = self.stsgcn(graph, normalized_spatial_adjacency_matrix, spatial_similarity)

        gcn_representation = gcn_representation.permute(0, 2, 1, 3)

        features = self.tcns[0](gcn_representation)
        for k in range(1, self.n_tcn):
            features = F.dropout(self.tcns[k](features) + features, p=self.dropout)

        prediction = self.output(features)   # prediction=[N, Tpred, nums, 2]
        prediction = self.multi_output(prediction.permute(0, 2, 1, 3))   # prediction=[N, 20, Tpred, 2]

        return prediction.permute(1, 2, 0, 3).contiguous()
